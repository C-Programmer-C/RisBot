...
async def upload_file_to_pyrus(file_path: Path, filename: str) -> Optional[str]:
    """Uploads a file from disk to Pyrus and returns its unique GUID"""
    try:
        with open(file_path, "rb") as f:
            file_bytes = BytesIO(f.read())
            file_bytes.seek(0)
            return await get_unique_file_id(file_bytes, filename)
    except Exception as e:
        logger.error(
            f"An error occurred while trying to upload file {file_path} to Pyrus: {e}"
        )
        return None
...
async def add_comment_with_attachment(
    ...
    text: str, 
    file_guid: str, 
    subscribers_removed: Optional[List[Dict[str, Any]]] = None
) -> bool:
    """Adds a comment with file attachment to a task in Pyrus"""
    try:
        json_data = {
            "text": text,
        ...
        if subscribers_removed:
            json_data["subscribers_removed"] = subscribers_removed
        
        result = await api_request(
            "POST", f"/tasks/{task_id}/comments", json_data=json_data
        )
        if isinstance(result, dict):
            logger.info(f"Comment with attachment added successfully to task {task_id}")
            return True
        return False
    except Exception as e:
        logger.error(
            f"An error occurred while trying to add comment with attachment to task {task_id}: {e}"
        )
        return False
